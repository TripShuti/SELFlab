...
- Усі host-шляхи в `volumes` задаються через env-змінні (див. `.env.example`
  кожної підпапки) — заміни `JELLYFIN_CONFIG`, `STORAGE_DIR` і т.д. на свої,
  і стек підніметься на твоїх шляхах без правки compose-файлів.
